...
import { Command } from "../utils/types/commands.ts";
import { needMessage } from "../utils/collectors.ts";
import { handleError } from "../utils/errors.ts";
import { logger } from "../utils/logger.ts";

const log = logger({ name: "Command Handler" });

export function parsePrefix(guildId: bigint | undefined) {
  const prefix = guildId ? cache.guildPrefixes.get(guildId) : configs.defaultPrefix;
  return prefix || configs.defaultPrefix;
}
...
async function parseArguments(
  ...
  for (const argument of command.arguments) {
    const resolver = cache.arguments.get(argument.type || "string");
    ...
    if (Object.prototype.hasOwnProperty.call(argument, "defaultValue")) {
      ...
      const question = await message
        .reply(
          `You were missing the ${argument.name} argument which is required in that command. Please provide the ${missingArg} now.`
        )
        .catch(log.error);
      if (question) {
...
async function commandAllowed(
  ...
  const inhibitorResults = await Promise.all(
    [...cache.inhibitors.values()].map((inhibitor) => inhibitor(message, command))
  );
...
async function executeCommand(
  ...
  try {
    // cache.slowmode.set(message.author.id, message.timestamp);
...
cache.monitors.set("commandHandler", {
  ...
  execute: async function (message) {
    // If the message was sent by a cache we can just ignore it
    ...
    // If the message is not using the valid prefix or cache mention cancel the command
    ...
    // const lastUsed = cache.slowmode.get(message.author.id);
