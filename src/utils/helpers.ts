...
import { ArgumentDefinition, Command } from './types/mod.ts';
...
export function createSubcommand<T extends readonly ArgumentDefinition[]>(
  ...
  let command: Command<T> = cache.commands.get(commandName)!;

  if (names.length > 1) {
    for (const name of names) {
      const validCommand = command ? command.subcommands?.get(name) : cache.commands.get(name);
...
export async function fetchMember(guildId: bigint, id: bigint | string) {
  ...
  const guild = discordCache.guilds.get(guildId);
  if (!guild) return;

  const cachedMember = discordCache.members.get(userId);
