...
createCommand({
  name: 'dev-ping',
  description: 'Pings the Bot! Returns extra information about performance impacts',
  botChannelPermissions: ["SEND_MESSAGES"],
  execute: (message) => {
    let memberCount = 0;
    ...
    message.send(`Ping MS: ${Date.now() - message.timestamp}ms | Guilds: ${discordCache.guilds.size} | Users: ${memberCount}`);
  }
})
